...
import {
  ...
  Users, Wifi, WifiOff, Wrench, RefreshCw,
} from 'lucide-react';
...
export default function ClusterPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    await createMutation.mutateAsync({ data: { ...form, port: form.port || 443, maxClients: form.maxClients || 100 } });
    invalidate();
    setIsAddOpen(false);
    setForm(defaultForm);
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    await updateMutation.mutateAsync({ id: editServer, data: form });
    invalidate();
    setEditServer(null);
    setForm(defaultForm);
  };
  ...
  const openEdit = (srv: { id: number; name: string; address: string; port: number; country: string; countryFlag: string; provider: string; maxClients: number }) => {
    setForm({ name: srv.name, address: srv.address, port: srv.port, country: srv.country, countryFlag: srv.countryFlag, provider: srv.provider, maxClients: srv.maxClients });
